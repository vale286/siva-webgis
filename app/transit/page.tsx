...
export default function TransitPage() {
  ...
  return (
    <main className="h-screen w-screen relative overflow-hidden flex flex-col">
      {/* Header Panel */}
      <div className={`absolute top-16 md:top-4 left-4 md:left-16 z-[9998] bg-white p-4 rounded shadow-lg border-l-4 border-blue-600 w-[calc(100vw-32px)] md:w-80 transition-all duration-300 ${showInfo ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0 pointer-events-none md:translate-y-0 md:opacity-100 md:pointer-events-auto'}`}>
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center">
            <img src="/assets/logo_siva.png" alt="SIVA Logo" className="w-8 h-8 object-contain mr-2" />
            <h2 className="font-bold text-lg text-blue-600">Transit to Trap</h2>
          </div>
          <button className="md:hidden text-gray-500 font-bold" onClick={() => setShowInfo(false)}>✕</button>
        </div>
        <p className="text-xs text-gray-700 leading-relaxed mb-2">
          <strong>Purpose:</strong> Maps main logistics corridors from entry points (ports/airports) to known high-risk complexes.
        </p>
        <p className="text-xs text-gray-700 leading-relaxed">
          <strong>Rescue Efforts:</strong> Establish checkpoints along highlighted transit paths. Increase screening at key ferry terminals and bus stations.
        </p>
      </div>
      ...
      {!showInfo && (
        <button 
          ...
          className="md:hidden absolute top-16 left-4 z-[9998] bg-white px-3 py-2 rounded shadow border-l-4 border-blue-600 text-sm font-bold flex items-center"
        >
